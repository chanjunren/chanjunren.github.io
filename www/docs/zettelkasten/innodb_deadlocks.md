...
MySQL thread id 1118382, OS thread handle 140387448583936, query id 286359804 [redacted-internal-ip] okcoin updating
update
    [redacted-internal-table]
...
RECORD LOCKS space id 26094 page no 48 n bits 784 index idx_uid of table `[redacted-internal-schema]`.`[redacted-internal-table]` trx id 8912811226 lock_mode X waiting
...
MySQL thread id 1118752, OS thread handle 140387430213376, query id 286360090 [redacted-internal-ip] okcoin updating
update
		 [redacted-internal-table]
		...
		deleted = 1
		 WHERE (  user_id = 7000000000123104
									...
									and id <> 2540 )
*** (2) HOLDS THE LOCK(S):
RECORD LOCKS space id 26094 page no 48 n bits 784 index idx_uid of table `[redacted-internal-schema]`.`[redacted-internal-table]` trx id 8912811219 lock_mode X locks rec but not gap
Record lock, heap no 713 PHYSICAL RECORD: n_fields 2; compact format; info bits 0
 0: len 8; hex 8018de76816f60e0; asc    v o` ;;
1: len 8; hex 80000000000009ec; asc         ;;

*** (2) WAITING FOR THIS LOCK TO BE GRANTED:
RECORD LOCKS space id 26094 page no 48 n bits 784 index idx_uid of table `[redacted-internal-schema]`.`[redacted-internal-table]` trx id 8912811219 lock_mode X waiting
Record lock, heap no 713 PHYSICAL RECORD: n_fields 2; compact format; info bits 0
 0: len 8; hex 8018de76816f60e0; asc    v o` ;;
 1: len 8; hex 80000000000009ec; asc         ;;
...
The output describes a deadlock scenario involving two transactions (identified as 8912811226 and 8912811219). Both transactions were attempting to update the `[redacted-internal-table]` table, leading to a conflict and ultimately a deadlock. MySQL resolved this by rolling back transaction (2).
...
- The SQL query it was executing was an `UPDATE` statement on the `[redacted-internal-table]` table, setting `modify_date` and `deleted` for a specific row (where `user_id = 7000000000123103` and `deleted = 0` and `id <> 2541`).
- It was waiting for an exclusive lock (lock_mode X) on a record in the `idx_uid` index of the `[redacted-internal-table]` table.
...
- The SQL query it was executing was also an `UPDATE` statement on the `[redacted-internal-table]` table, setting `modify_date` and `deleted` for a specific row (where `user_id = 7000000000123104` and `deleted = 0` and `id <> 2540`).
...
- Both transactions were trying to update the same record (or at least records that fall within the same index range) in the `[redacted-internal-table]` table, leading to the lock conflict
