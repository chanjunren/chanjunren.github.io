...
export function CategoryTag({ children, markerColor }: { children: ReactNode; markerColor?: string }) {
  return (
    <CustomTag color="neutral" className="inline-flex! items-center gap-1.5 text-sm! font-normal">
      {markerColor && <span aria-hidden="true" className="size-2 rounded-full" style={{ backgroundColor: markerColor }} />}
      {children}
    </CustomTag>
  );
}
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button type="button" className="w-fit border-0 bg-transparent p-0">
          <CustomTag color="rose" className="text-base! font-semibold">うち</CustomTag>
        </button>
      </TooltipTrigger>
      <TooltipContent side="right" sideOffset={8} className="max-w-64 leading-relaxed">
        うち (uchi)<br />meaning home, inside, or one&apos;s inner circle<br /><br />my secret projects
      </TooltipContent>
    </Tooltip>
  );
}

export function KakeiboSidebar() {
  return (
    <aside className="relative hidden w-72 shrink-0 bg-background lg:flex lg:flex-col">
      ...
      <div className="px-6 pt-8"><Wordmark /></div>
